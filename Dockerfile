...
# RUN apk update
# RUN apk add gcc g++ autoconf postgresql-dev
# RUN ./configure --enable-pthreads=shared
...
RUN chmod 777 /usr/local/bin/docker-php-ext-configure
RUN chmod 777 /usr/local/bin/docker-php-ext-enable
RUN chmod 777 /usr/local/bin/docker-php-ext-install
...
RUN apk add --update libpthread-stubs make git
...
RUN echo "extension=pthreads.so" >> "/usr/local/etc/php/php-cli.ini" \
